...
export const PLANET_FRAGMENT_SHADER = /* glsl */ `
  ...
  void main() {
    ...
    // Continents drift because the noise domain is offset by geological time
    // rather than reseeded per era. Two eras a few million years apart barely
    // move; four billion years rearranges the map.
    vec3 drift = vec3(tectonic * 1.9, tectonic * 0.5, tectonic * -1.2);
    // Each warp component needs its own offset *and* its own frequency.
    // Offsetting one field along the diagonal gives three correlated results,
    // which warps the domain along a single axis and marbles the whole globe.
    vec3 warp = vec3(
    ...
    float ranges = ridged3(terrainPoint * 2.6);
    ...
    float sea = 1.0 - smoothstep(oceanCover - 0.012, oceanCover + 0.018, rank);
    float land = smoothstep(oceanCover, min(1.0, oceanCover + 0.40), rank);
    ...
    float craterBasin = smoothstep(0.25, 0.75, fbm3(surfacePoint * 6.4 + 19.0) * 0.5 + 0.5);
    ...
    vec3 albedo = mix(ground, water, sea);
    ...
    // Molten worlds: a mostly-solid crust broken by an incandescent network.
    // The crust has to be genuinely dark for this to read as molten rock —
    // lighting bright orange from every direction just gives a beige ball,
    // because a filmic tone curve desaturates anything that bright.
    float flow = fbm4(terrainPoint * 2.4 + 31.0) * 0.5 + 0.5;
    float fissure = pow(ridged3(terrainPoint * 5.2), 6.0) * fineDetail;
    float crust = smoothstep(0.18, 0.56, fbm4(terrainPoint * 3.1 + 53.0) * 0.5 + 0.5);
    vec3 magma = mix(vec3(0.90, 0.13, 0.02), vec3(1.0, 0.55, 0.10), flow);
    ...
    float exposedMelt = molten * max(1.0 - crust, fissure);
    ...
    float caps = smoothstep(capEdge - 0.10, capEdge + 0.07, latitude + (iceGrain - 0.5) * 0.17);
    ...
    float scar = 1.0 - smoothstep(0.05, 0.30, distance(surfacePoint, normalize(vec3(0.62, 0.34, 0.71))));
    albedo = mix(albedo, vec3(1.0, 0.52, 0.13), impactFlash * pow(scar, 2.4) * 0.85);

    // ---- the photographic present ---------------------------------------
    // Only the present-day era carries these maps; everywhere else
    // referenceMap is zero and the samplers cost a lookup that is thrown away.
    ...
    // range of thresholds is narrow and sits just above that: a little too low
    // and the whole planet roofs over.
    float threshold = 0.62 - cloudCover * 0.22;
    float proceduralCloud = smoothstep(threshold, threshold + 0.12, density) *
    ...
    float photographCloud = texture2D(
      cloudMap,
      vec2(fract(vUv.x + cloudDrift * 0.04), vUv.y)
    ).a;
